...
import type { Order, DeliveryChallan } from '../../types'
...
// ─── Main Component ───────────────────────────────────────────────────────────
export function DeliveryChallanPDFDocument({ order, challan: propChallan, settings: propSettings }: { order: Order; challan?: DeliveryChallan; settings?: any }) {
  const settings = propSettings || fetchSettings()
  const challan = propChallan || order.delivery_challan

  let devStyles: any = {}
  try {
    if (settings.challan_developer_styles) devStyles = JSON.parse(settings.challan_developer_styles)
  } catch {}

  const themeColor = settings.challan_theme_color || '#115e59'
  const accentColor = settings.challan_accent_color || '#f0fdf9'
  const headerTextColor = settings.challan_header_text_color || '#FFFFFF'
  ...
  const grandTotal = items.reduce((s, i) => s + safeNum(i.subtotal), 0)
  ...
  const challanNumber = challan?.challan_number || 'DC-2026-001'
  const challanDate = formatDate(challan?.delivery_date)
  const challanType = challan?.challan_type === 'returnable' ? 'Returnable' : 'Non-Returnable'

  const visibleCols = settings.challan_visible_columns || ['sl', 'desc', 'hsn', 'orderedQty', 'deliveredQty', 'unitPrice', 'subtotal']
  ...
  Object.keys(defaultColWidths).forEach((colId) => {
    if (colId !== 'desc' && !hasCol(colId)) descWidth += defaultColWidths[colId]
  })
  ...
  const resolveNoteTags = (text?: string) => {
    ...
    return text
      ...
      .replace(/#Date#/g, challanDate)
      .replace(/#Company#/g, companyName)
      .replace(/#Total#/g, formatCurrency(grandTotal))
      .replace(/#ChallanNo#/g, challanNumber)
  }

  const headerNote = settings.challan_custom_header_note
  const footerNote = settings.challan_custom_footer_note

  return (
    <Document title={`Delivery Challan - ${challanNumber}`} author={companyName}>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.titleBanner}>
          <Text style={styles.titleBannerText}>Delivery Challan</Text>
          <Text style={styles.titleBannerSub}>{challanType}</Text>
        </View>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.metaBox}>
            ...
            <View style={styles.metaRow}>
              ...
              <Text style={styles.metaValue}>{challanDate}</Text>
            </View>
            {order.order_number && (
              <View style={styles.metaRow}>
            ...
            <View style={styles.metaRow}>
              ...
              <Text style={styles.metaValue}>{challanType}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.splitRow}>
          <View style={styles.addressBlock}>
            <Text style={styles.boxLabel}>Dispatched From</Text>
          ...
          <View style={styles.addressBlock}>
            <Text style={styles.boxLabel}>Delivered To (Consignee)</Text>
            <Text style={styles.addressName}>{order.customer_name || 'Client'}</Text>
            {challan?.delivery_address
              ? <Text style={styles.addressText}>{challan.delivery_address}</Text>
          ...
          <View style={styles.metaBlock}>
            {challan?.vehicle_number ? (
              <View style={styles.metaItem}>
                ...
                <Text style={styles.metaItemValue}>{challan.vehicle_number}</Text>
              </View>
            ) : null}
            {challan?.driver_name ? (
              <View style={styles.metaItem}>
                <Text style={styles.metaItemLabel}>Driver</Text>
                <Text style={styles.metaItemValue}>{challan.driver_name}</Text>
              </View>
            ) : null}
            {challan?.driver_contact ? (
              <View style={styles.metaItem}>
                <Text style={styles.metaItemLabel}>Contact</Text>
                <Text style={styles.metaItemValue}>{challan.driver_contact}</Text>
              </View>
            ) : null}
            {challan?.expected_delivery_date ? (
              <View style={[styles.metaItem, { borderBottomWidth: 0 }]}>
                <Text style={styles.metaItemLabel}>Exp. Delivery</Text>
                <Text style={styles.metaItemValue}>{formatDate(challan.expected_delivery_date)}</Text>
              </View>
            ) : null}
          </View>
        </View>
        ...
        {/* Items Table */}
        <Text style={styles.tableTitle}>Delivery Challan Items</Text>
        <View style={styles.table}>
          <View style={styles.tableHeaderRow}>
            ...
            {hasCol('hsn') && <Text style={[styles.tableHeaderCell, getColWidth('hsn'), { textAlign: 'center' }]}>HSN Code</Text>}
            {hasCol('orderedQty') && <Text style={[styles.tableHeaderCell, getColWidth('orderedQty'), { textAlign: 'center' }]}>Ord. Qty</Text>}
            {hasCol('deliveredQty') && <Text style={[styles.tableHeaderCell, getColWidth('deliveredQty'), { textAlign: 'center' }]}>Del. Qty</Text>}
            {hasCol('unitPrice') && <Text style={[styles.tableHeaderCell, getColWidth('unitPrice'), { textAlign: 'right' }]}>Unit Price</Text>}
            {hasCol('subtotal') && <Text style={[styles.tableHeaderCell, getColWidth('subtotal'), { textAlign: 'right', borderRightWidth: 0 }]}>Subtotal</Text>}
          </View>
          {items.map((item, idx) => {
            const isAlt = idx % 2 === 1
            return (
              <View key={idx} style={isAlt ? styles.tableBodyRowAlt : styles.tableBodyRow}>
                ...
                {hasCol('hsn') && <Text style={[styles.tableCell, getColWidth('hsn'), { textAlign: 'center', color: '#555555' }]}>{item.hsn_code || '—'}</Text>}
                {hasCol('orderedQty') && <Text style={[styles.tableCell, getColWidth('orderedQty'), { textAlign: 'center' }]}>{item.quantity}</Text>}
                {hasCol('deliveredQty') && <Text style={[styles.tableCell, getColWidth('deliveredQty'), { textAlign: 'center', fontFamily: 'Helvetica-Bold' }]}>{item.delivered_quantity}</Text>}
                {hasCol('unitPrice') && <Text style={[styles.tableCell, getColWidth('unitPrice'), { textAlign: 'right' }]}>{formatCurrency(item.unit_price)}</Text>}
                {hasCol('subtotal') && <Text style={[styles.tableCell, getColWidth('subtotal'), { textAlign: 'right', borderRightWidth: 0, fontFamily: 'Helvetica-Bold' }]}>{formatCurrency(item.subtotal)}</Text>}
              </View>
            )
          })}
        </View>
        ...
        <View style={styles.totalsBox}>
          <View style={styles.totalsHeaderRow}>
            <Text style={styles.totalsHeaderText}>Challan Value Summary</Text>
          </View>
          ...
          <View style={styles.totalsRow}>
            <Text style={styles.totalsLabel}>Total Qty Delivered</Text>
            <Text style={styles.totalsValue}>{items.reduce((s, i) => s + safeNum(i.delivered_quantity), 0)}</Text>
          </View>
          <View style={styles.grandTotalRow}>
            <Text style={styles.grandTotalLabel}>Total Value (INR)</Text>
        ...
        {/* Additional Notes */}
        {challan?.additional_notes && (
          <View style={styles.termsBox}>
            <Text style={styles.boxLabel}>Additional Notes</Text>
            <Text style={styles.termsText}>{challan.additional_notes}</Text>
          </View>
        )}
        ...
        <View style={styles.signatureRow}>
          <View style={styles.signatureBox}>
            ...
            <Text style={styles.signatureFooter}>Authorized Signatory & Stamp</Text>
          </View>
          <View style={styles.signatureBox}>
            <Text style={styles.signatureLabel}>Received By (Consignee)</Text>
            <Text style={styles.signatureTitle}>{order.customer_name || '—'}</Text>
            <Text style={styles.signatureFooter}>Signature & Date of Receipt</Text>
          </View>
          <View style={styles.signatureBox}>
            <Text style={styles.signatureLabel}>Driver Acknowledgment</Text>
            <Text style={styles.signatureTitle}>{challan?.driver_name || '—'}</Text>
            <Text style={styles.signatureFooter}>Driver Signature</Text>
          </View>
        </View>
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.footerLeft}>
            {footerNote ? resolveNoteTags(footerNote) : `Challan: ${challanNumber} | ${companyName}`}
          </Text>
