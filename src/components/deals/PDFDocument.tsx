...
export function QuotePDFDocument({ deal, settings: propSettings }: { deal: Deal; settings?: any }) {
  ...
  // ── Auto-generate BOM from sub_items if no explicit BOM data ──────────────────
    ...
    if (item.sub_items && item.sub_items.length > 0) {
      item.sub_items.forEach(sub => {
        autoGeneratedBom.push({
          section: 'Untitled',
          module: item.product_name || 'Product',
          description: sub.description,
          quantity: sub.quantity * safeNum(item.quantity),
  ...
  const bomData: BOMItem[] = (deal.bom_data && deal.bom_data.length > 0) ? deal.bom_data : autoGeneratedBom
  const showBOM = bomData.length > 0
  ...
  const renderTitleBanner = () => (
    <View key="titleBanner" style={styles.titleBanner}>
      <Text style={styles.titleBannerText}>Commercial Proposal</Text>
    </View>
  )
  ...
  return (
    <Document>
      ...
      {/* ═══════════════════════════════════════════════════════════════════════
          PAGE 2 – ANNEXURE 1: BILL OF MATERIAL (only if BOM data exists)
      ═══════════════════════════════════════════════════════════════════════ */}
      {showBOM && (
        <Page size="A4" style={styles.annexurePage}>
          ...
          <View style={styles.annexureHeaderRow}>
            <View style={styles.annexureTitleBlock}>
              ...
              <Text style={styles.annexureSubtitle}>Technical Specifications &amp; Product Bundle Components</Text>
            </View>
            <View style={styles.annexureMetaBlock}>
              <Text style={styles.annexureMetaText}>Quote Ref: {quoteNo}</Text>
